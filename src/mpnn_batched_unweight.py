...
class UgiLoaderFast:
    ...
    """
    def __init__(self, csv_path, inds, batch_size=32, shuffle=False):
        """
    ...
    def __next__(self):
        ...
        #return batch
        return df_batch['smiles'].values, df_batch['dock_score'].to_numpy().reshape(-1,1)
...
def main(args):
    ...
    train_loader = UgiLoaderFast(args.path, inds=train_ind, batch_size=args.batch_size, shuffle=True)
    if args.test:
        test_loader = UgiLoaderFast(args.path, inds=test_ind, batch_size=args.batch_size, shuffle=False)

    if args.val_path is not None:
        val_ind = range(len(pd.read_csv(args.val_path)))
        val_loader = UgiLoaderFast(args.val_path, inds=val_ind, batch_size=args.batch_size, shuffle=False)
    ...
    for epoch in range(start_epoch, args.n_epochs+1):
        ...
        for i, (smiles, labels) in tqdm(enumerate(train_loader, start=start_batch), initial=start_batch, total=int(len(train_ind)/args.batch_size)):
            ...
            batch_preds = y_pred.cpu().detach().numpy()
            batch_labs = labels.cpu().detach().numpy()
            ...
            if (i!=0 and i%args.write_batch==0) or epoch%args.write_batch==0:
                ...
                if args.val_path is not None:
                    ...
                    for j, (smiles, labels) in enumerate(val_loader):
                        ...
                        val_labs[m: m + len(smiles) ] = batch_labs_val.reshape(len(batch_labs_val),1)
                        m += len(smiles)
