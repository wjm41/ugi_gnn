...
def main(args):
    ...
    if args.val:
        train_ind, val_ind, y_train, _ = train_test_split(
            range(len_df), y_train, test_size=args.val_set_size, random_state=42)
    else:
        train_ind = range(len_df)

    y_scaler = y_scaler.fit(y_train.reshape(-1, 1))

    train_loader = UgiLoaderFast(
        args.path, inds=train_ind, batch_size=args.batch_size, shuffle=True, y_scaler=y_scaler)
    val_loader = None
    if args.val:
        val_loader = UgiLoaderFast(
...
if __name__ == '__main__':
    ...
    group_data.add_argument('-val', action='store_true',
                            help='whether or not to do train/val split and log val_loss')
    group_data.add_argument('-val_set_size', type=float, default=0.2,
                            help='Proportion of training set to use as validation set')
