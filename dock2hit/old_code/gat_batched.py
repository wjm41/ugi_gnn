...
def main(args):
    ...
    num_gnn_layers = 2
    gat_net = GATPredictor(in_feats=39,
    ...
    loss_fn = MSELoss()
    optimizer = torch.optim.Adam(gat_net.parameters(), lr=args.lr)
    ...
    if args.load_name is not None:
       ...
       gat_net.load_state_dict(checkpoint['mpnn_state_dict'])
    ...
    gat_net = gat_net.to(device)
    print('Number of parameters: {}'.format(sum(p.numel() for p in gat_net.parameters() if p.requires_grad)))
    ...
    for epoch in range(start_epoch, args.n_epochs+1):
        ...
        for i, (smiles, labels) in tqdm(enumerate(train_loader, start=start_batch), initial=start_batch, total=int(len(train_ind)/args.batch_size)):
            ...
            if (i!=0 and i%args.save_batch==0) or epoch%args.write_batch==0:
                try:
                    torch.save({
                                'epoch': epoch,
                                'mpnn_state_dict': gat_net.state_dict(),
        ...
        if args.test:
            ...
            for i, (smiles, labels) in tqdm(enumerate(test_loader), total=int(len(test_ind)/args.batch_size)):
                ...
                bond_feats = bg.edata.pop('e').to(device)
                atom_feats, bond_feats, labels = atom_feats.to(device), bond_feats.to(device), torch.tensor(labels, dtype=torch.float32).to(device)
                y_pred = gat_net(bg, atom_feats, bond_feats)
